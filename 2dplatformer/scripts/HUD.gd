...
func tutorial(mechanic):
	...
	if mechanic == "movement":
		...
		text = text.replace("(Physical)", "")
		$Control/tutorials.text = "press: %s to cast assigned spells (shown under health bar)"%text
	
	elif mechanic == "keys":
		$Control/tutorials.text = "green items are keys, opens a locked door somewhere"
	
	elif mechanic == "movement_unlocks":
		$Control/tutorials.text = "gold items are movement abilities which let you explore new areas"
	
	elif mechanic == "spell_unlocks":
		$Control/tutorials.text = "blue items are spells, check which spells are unlocked in the inventory"
